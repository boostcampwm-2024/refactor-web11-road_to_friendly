...
import UserKeywords from '@/routes/components/statistics/UserKeywords';
...
const UserProfile = React.memo(({ participant, isCurrentUser, isHost, position }: UserProfileProps) => {
  const [isUserKeywordsShown, setUserKeywordsShown] = useState(false);
  const { radius, isOutOfBounds, increaseLongRadius } = useRadiusStore();
  const location = useLocation();
  const profileStyles = PROFILE_STYLES[(participant?.index || 0) % PROFILE_STYLES.length];

  useEffect(() => {
    setUserKeywordsShown(['/statistics', '/content-share'].some((path) => location.pathname.endsWith(path)));
  }, [location.pathname]);

  useEffect(() => {
    if (isUserKeywordsShown) {
      increaseLongRadius();
    }
  }, [isUserKeywordsShown]);
  ...
  return (
    <div css={profileStyle(position.x, position.y, radius[0], radius[1], isOutOfBounds)}>
      ...
      {isUserKeywordsShown && <UserKeywords participant={participant} />}
    </div>
  );
}, areEqual);
